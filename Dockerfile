...
COPY .docker/assets/startup.sh /startup.sh
COPY .docker/assets/self-test.sh /self-test.sh
...
COPY .docker/assets/monolog-dev.yaml /opt/kimai/config/packages/dev/monolog.yaml
...
COPY .docker/assets/monolog-prod.yaml /opt/kimai/config/packages/prod/monolog.yaml
# do the composer deps installation
RUN export COMPOSER_HOME=/composer && \
